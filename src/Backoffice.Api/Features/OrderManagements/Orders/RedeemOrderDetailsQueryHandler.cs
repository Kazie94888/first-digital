...
internal class RedeemOrderDetailsQueryHandler : IQueryHandler<RedeemOrderDetailsQuery, RedeemOrderDetailsResponse>
{
    ...
    {
        var order = await _context.Orders.FirstAsync(o => o.Id == request.OrderId, cancellationToken);
        ...
        var response = new RedeemOrderDetailsResponse
        {
            ...
            DepositDetails = await GetRedeemDepositDetailsAsync(request, cancellationToken),
    ...
    private async Task<RedeemOrderSummary> GetOrderSummaryAsync(RedeemOrderDetailsQuery request,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<RedeemDepositDetails> GetRedeemDepositDetailsAsync(RedeemOrderDetailsQuery request,
        CancellationToken cancellationToken)
    {
        var queryResult = await (
            from order in _context.Orders
        ...
        CancellationToken cancellationToken)
    {
        var queryResult = await (
            ...
            select new RedeemBurningDetails
            {
                ...
                FromWallet = new BlockchainAddressDto
                {
                    Address = orderWallet.WalletAccount.Address,
                    Network = orderWallet.WalletAccount.Network,
                },
    ...
    private async Task<RedeemBankDetails> GetBankDetailsAsync(OrderId orderId, CancellationToken cancellationToken)
    {
        var queryResult = await (
            ...
            {
                OrderBank = orderBank,
                OrderBankAddress = orderBankAddress,
    ...
    private async Task<RedeemRsnDetails> GetRsnDetailsAsync(OrderId orderId, CancellationToken cancellationToken)
    {
        var queryResult = await (
            from order in _context.Orders
            join orderFdtDepositAccount in _context.FdtDepositAccounts on order.DepositFdtAccountId equals orderFdtDepositAccount.Id
            ...
            select new RedeemRsnDetails
            {
                ...
                {
                    Name = orderFdtDepositAccount.AccountName,
                    Number = orderFdtDepositAccount.AccountNumber
                },
                ToFdtAccount = new FdtAccountDto
                {
                    Name = orderFdtAccount.ClientName,
                    Number = orderFdtAccount.AccountNumber,
                },
    ...
    private static List<RedeemProgressOverview> GetProgressOverview(Order order)
    {
        ...
        [
            ...
            {
                Title = RedeemProgressOverviewTitle.Ordered,
                Status = RedeemProgressOverviewStatus.Completed,
                Amount = new MoneyDto
                {
                    Amount = order.OrderedAmount.Amount,
                    AssetSymbol = order.OrderedAmount.Currency
            ...
            new RedeemProgressOverview
            {
                Title = RedeemProgressOverviewTitle.Deposited,
            ...
            new RedeemProgressOverview
            {
                Title = RedeemProgressOverviewTitle.Burned,
            ...
            new RedeemProgressOverview
            {
                Title = RedeemProgressOverviewTitle.FiatWithrawal,
                Status = GetFiatWithdrawalProgressStatus(order),
                Amount = order.ActualAmount == null
                    ? null
                    : new MoneyDto
                    {
    ...
    private static RedeemProgressOverviewStatus GetDepositedProgressStatus(Order order)
    {
        if (order.ProcessingStatus == OrderProcessingStatus.DepositTxHashSet)
            return RedeemProgressOverviewStatus.Upcoming;
    ...
    private static RedeemProgressOverviewStatus GetFiatWithdrawalProgressStatus(Order order)
    {
        return order.ProcessingStatus switch
        {
            OrderProcessingStatus.RsnReferenceCreated => RedeemProgressOverviewStatus.Upcoming,
