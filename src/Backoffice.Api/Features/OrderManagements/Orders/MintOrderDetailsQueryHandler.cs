...
internal class MintOrderDetailsQueryHandler : IQueryHandler<MintOrderDetailsQuery, MintOrderDetailsResponse>
{
    ...
    public async Task<Result<MintOrderDetailsResponse>> Handle(MintOrderDetailsQuery request, CancellationToken cancellationToken)
    {
        var order = await _context.Orders.FirstAsync(o => o.Id == request.OrderId, cancellationToken: cancellationToken);
    ...
    private async Task<MintOrderSummary> GetOrderSummaryAsync(MintOrderDetailsQuery request, CancellationToken cancellationToken)
    {
    ...
    private async Task<MintRsnDetails> GetRsnDetailsAsync(OrderId orderId, CancellationToken cancellationToken)
    {
        var depositResult = await (from order in _context.Orders
                                   ...
                                   join toDepositFdt in _context.FdtDepositAccounts on order.DepositFdtAccountId equals toDepositFdt.Id into depositGroup
        ...
        {
            FromFdtAccount = new FdtAccountDto
            {
    ...
    private async Task<MintingDetails> GetMintingDetailsAsync(OrderId orderId, CancellationToken cancellationToken)
    {
        var mintingResult = await (from order in _context.Orders
                                   join orderWallet in _context.Wallets on order.WalletId equals orderWallet.Id
                                   join depositWallet in _context.DepositWallets on order.DepositWalletId equals depositWallet.Id into depositWalletGroup
        ...
        {
            DepositAmount = mintingResult.DepositedAmount == null ? null : new MoneyDto
            {
        ...
        var processingStatusWithoutAmount = new List<OrderProcessingStatus> { OrderProcessingStatus.DepositInstructionCreated, OrderProcessingStatus.RsnReferenceCreated };

        if (mintingResult.ProcessingStatus.HasValue
            && !processingStatusWithoutAmount.Exists(x => x == mintingResult.ProcessingStatus)
            && mintingResult.MintingAmount is not null)
        {
            mintingDetails.MintingAmount = new MoneyDto
            {
                Amount = mintingResult.MintingAmount.Amount,
                AssetSymbol = mintingResult.MintingAmount.Currency
            };
        }
    ...
    private static List<MintProgressOverview> GetProgressOverview(Order order)
    {
        ...
        [
            ...
            {
                Title = MintProgressOverviewTitle.Ordered,
                Status = MintProgressOverviewStatus.Completed,
                Amount = new MoneyDto
                {
                    Amount = order.OrderedAmount.Amount,
                    AssetSymbol = order.OrderedAmount.Currency
            ...
            new MintProgressOverview
            {
                Title = MintProgressOverviewTitle.Deposited,
                Status = GetDepositedProgressStatus(order),
            ...
            new MintProgressOverview
            {
                Title = MintProgressOverviewTitle.Minted,
    ...
    private static MintProgressOverviewStatus GetDepositedProgressStatus(Order order)
    {
        ...
        if (upcomingDepositState.Exists(x => x == order.ProcessingStatus))
            return MintProgressOverviewStatus.Upcoming;
    ...
    private static MintProgressOverviewStatus GetMintProgressStatus(Order order)
    {
        ...
        {
            OrderProcessingStatus.SigningInitiated => MintProgressOverviewStatus.Upcoming,
